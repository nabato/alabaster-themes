(ns com.vlnabatov.alabaster.factory
  ...
  (:gen-class :main false
              ...
              :name com.vlnabatov.alabaster.Factory))

(deftype PortalLogger [])

(def LOG (Logger/getInstance PortalLogger))

(defn info [message] (.info ^Logger LOG ^String message))
...
(defn- get-options
  []
  (pr-str (pr-str) {:theme ::theme} :themes {::theme (theme/get-theme)}))
...
(defn start
  ...
  (swap! instances update
    project
    (fn [m]
      (cond-> m
        (not (:server m)) (assoc :server
                            (http/run-server #(handler % project)
                                             {:port 0,
                                              :legacy-return-value? false})))))
  (write-config project
                {:host "localhost"}
                :port
                (http/server-port (get-in @instances [project :server]))))
...
(defn- setup-java-error-handler
  [^JBCefBrowser browser]
  (doto (JBCefJSQuery/create browser)
    (.addHandler (as-function #(throw (ex-info (str "JavaScript error: " %)
                                               {}))))))

(defn- inject-js-error-handler
  [^JBCefBrowser browser ^JBCefJSQuery js-query]
...
(defn- setup-load-handler
  [^JBCefBrowser browser js-query]
  (.addLoadHandler (.getJBCefClient browser)
                   (reify
                     CefLoadHandler
                       (onLoadingStateChange [_this _browser _isLoading
                       ...
                       (onLoadError [_this _browser _frame errorCode errorText
                                     failedUrl]
                         (throw (ex-info errorText
                                         {:errorCode errorCode}
                                         :errorText errorText
                                         :failedUrl failedUrl))))
...
(defn get-nrepl
  []
  (try (requiring-resolve 'nrepl.server/start-server) (catch Exception _e)))
...
(defn -createToolWindowContent
  [_this ^Project project ^ToolWindow window]
