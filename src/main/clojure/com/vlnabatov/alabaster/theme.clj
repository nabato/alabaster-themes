(ns com.vlnabatov.alabaster.theme
  (:import (com.intellij.openapi.editor DefaultLanguageHighlighterColors)
           (com.intellij.openapi.editor.colors ColorKey
                                               EditorColorsManager
                                               EditorColorsScheme
                                               TextAttributesKey)
...
(def ^:private language-colors
  {:INLINE_PARAMETER_HINT
   ...
   :INLINE_REFACTORING_SETTINGS_DEFAULT
     DefaultLanguageHighlighterColors/INLINE_REFACTORING_SETTINGS_DEFAULT,
   ...
   :INLINE_REFACTORING_SETTINGS_FOCUSED
     DefaultLanguageHighlighterColors/INLINE_REFACTORING_SETTINGS_FOCUSED,
   ...
   :INLINE_REFACTORING_SETTINGS_HOVERED
     DefaultLanguageHighlighterColors/INLINE_REFACTORING_SETTINGS_HOVERED,
...
(defn- get-color
  ([key] (get-color (.getGlobalScheme (EditorColorsManager/getInstance)) key))
  ([^EditorColorsScheme theme key]
   (let [defaults (.getScheme (EditorColorsManager/getInstance)
                              EditorColorsManager/DEFAULT_SCHEME_NAME)]
     (some-> (if-let [attrs (language-colors key)]
               (cond (instance? ColorKey attrs) (.getColor theme
                                                           ^ColorKey attrs)
                     (instance? TextAttributesKey attrs)
                       (or (some-> theme
                                   (.getAttributes ^TextAttributesKey attrs)
                                   .getForegroundColor)
                           (some-> defaults
                                   (.getAttributes ^TextAttributesKey attrs)
                                   .getForegroundColor)))
...
(defn- get-font
  ([] (get-font (.getGlobalScheme (EditorColorsManager/getInstance))))
...
(defn resolve-theme
  [m]
  (reduce-kv (fn [out k v] (assoc out k (get-color v))) {} m))
